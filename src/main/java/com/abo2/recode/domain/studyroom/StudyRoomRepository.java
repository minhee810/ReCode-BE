...
public interface StudyRoomRepository extends JpaRepository<StudyRoom, Long> {
    ...
    @Query(value = "SELECT studyMemberId " +
            "FROM Study_Member WHERE studyId=:studyId AND user_id=:userId", nativeQuery = true)
    Long findIdByuserIdAndstudyId(@Param(value = "studyId") Long studyId,
                                  @Param(value = "userId") Long userId);

    @Query(value = "SELECT created_by FROM Study_Room s WHERE s.studyId=:studyId", nativeQuery = true)
    ...
    @Modifying
    @Query(value = "UPDATE Study_Room s SET s.created_by = :userId WHERE s.studyId = :studyId", nativeQuery = true)
    void memberRolePromote(@Param("studyId") Long studyId, @Param("userId") Long userId);

    @Modifying
    @Query(value = "UPDATE Study_Room s SET s.created_by = null WHERE s.studyId = :studyId", nativeQuery = true)
