...
public interface ChatRoomUserLinkRepository extends JpaRepository<ChatRoomUserLink,Long> {
    @Query(value = "SELECT chat_room_id as chatRoomId FROM ChatRoomUserLink WHERE user_id =: userId",nativeQuery = true)
    List<Long> getChatRoomsByUser(@Param(value = "userId") Long userId);

    @Query(value = "SELECT user_id as userId FROM ChatRoomUserLink WHERE chat_room_id =: chatRoomId",nativeQuery = true)
